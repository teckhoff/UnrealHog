# UNREAL-001: UnrealHog Blueprint Library
...
Create a Blueprint Function Library that condenses multi-node logic into a convenient single node.
...
- When this task is completed, update its **State** to `Completed` and replace its status icon in `Design/Planning/MakeItUnreal/README.md` with `✅`.
